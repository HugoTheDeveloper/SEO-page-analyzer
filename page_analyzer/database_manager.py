...
class DbManager:
    ...
    def insert_url_check(self, url_id, response, h1, title, content):
        with init_connection() as conn:
            with get_cursor(conn) as cursor:
                date = datetime.date.today()
                cursor.execute("INSERT INTO urls_checks (url_id, created_at, response_code, h1, title, description) VALUES (%s, %s, %s, %s, %s, %s)",
    ...
    def get_urls_list(self):
        with init_connection() as conn:
            with get_cursor(conn) as cursor:
                cursor.execute("SELECT DISTINCT url_id, max(id) AS id FROM urls_checks GROUP BY url_id")
                ...
                cursor.execute("SELECT urls.id AS id, urls.name AS name, "
                               ...
                               "FROM urls LEFT JOIN last_checks ON urls.id = last_checks.url_id "
                               "ORDER BY id DESC")
